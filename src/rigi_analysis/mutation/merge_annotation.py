...
def load_and_merge_data(annotated_dir, pattern_dir, mutation_type, dose, chromosomes=None, output_dir=None):
    ...
    if not chromosomes:
        ...
        if not pattern_files:
            ...
            if not pattern_files:
                print("No pattern files found for alternate path either")
                return pd.DataFrame()
    ...
    for chrom in chromosomes:
        ...
        print("Created MatchKey from MutationID by keeping only first 4 components")
        ...
        if len(components) >= 4:
            ...
            if all(col in pattern_df.columns for col in ['Chromosome', 'Start', 'Ref', 'Alt']):
                pattern_df['MatchKey'] = pattern_df['Chromosome'] + '_' + \
                                         ...
                                         pattern_df['Ref'] + '_' + \
                                         pattern_df['Alt']
                print("Built MatchKey from individual columns")
            else:
                print("Cannot create matching key: missing necessary columns in pattern file")
                continue
...
def analyze_specific_patterns(merged_df, output_dir, mutation_type, dose):
    ...
    if merged_df.empty or 'Pattern' not in merged_df.columns:
        print("Cannot analyze specific patterns: missing required columns")
        return
    ...
    if pattern_data.empty:
        print("No specific radiation patterns found in the data")
        return
