...
def load_all_data(annotsv_dir):
    ...
    for f in files:
        ...
        try:
            ...
            unique_svs['SV_type'].value_counts()
...
def create_figure(stats, output):
    ...
    ax1.barh(range(len(sv_order_sorted)), [total_counts[sv] for sv in sv_order_sorted],
    ...
    heatmap_data = stats['dose_week_total'].fillna(0).astype(int)
