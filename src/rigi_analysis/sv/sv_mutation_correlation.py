...
def analyze_pattern_concordance(results_df, output_dir):
    ...
    if total_rad > 0:
        ...
        if pval < 0.05 and concordant > expected_conc:
            print("    → SIGNIFICANT: Higher concordance than expected")
            print("    → Supports: Same repair event causes both SV and mutations")
...
def main():
    ...
    if len(sw_rad) > 0 and rad_enrich > 1.5:
        print("\n  ✓ SUPPORTS HYPOTHESIS:")
        print("    Radiation mutations cluster near SV breakpoints")
        print("    → Error-prone repair during SV formation causes mutations")
    elif len(sw_rad) > 0 and rad_enrich <= 1:
        print("\n  ✗ DOES NOT SUPPORT HYPOTHESIS:")
        print("    No enrichment of mutations near breakpoints")
