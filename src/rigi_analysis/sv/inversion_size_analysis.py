...
def test_mega_vs_small(df, threshold=50_000_000):
    ...
    if len(mega) > 0 and len(small) > 0:
        ...
        if p_value < 0.05:
            if odds_ratio > 1:
                print("  ✓ Mega-inversions SIGNIFICANTLY more enriched for DBS")
            else:
                print("  ✓ Smaller inversions SIGNIFICANTLY more enriched for DBS")
        else:
...
def main():
    ...
    if mega_stats:
        pd.DataFrame([mega_stats]).to_csv(
            output_dir / 'mega_test_results.csv', index=False
        )
