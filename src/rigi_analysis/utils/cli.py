...
This module provides the central entry point `rigi-analysis-run` that
dynamically loads and executes individual scripts from the `mutation`, `sv`,
`workflows`, or `utils` modules.
"""
...
def main():
    ...
    Raises:
        SystemExit: If no script name is provided, if the script cannot
            be found, or if the target script does not define a `main()`
            function.
    """
    if len(sys.argv) < 2:
        print("Usage: rigi-analysis-run <script_name> [args...]")
        print(
            "\nAvailable modules are dynamically resolved from mutation, "
            "sv, workflows, or utils."
        )
        sys.exit(1)
    ...
    for ns in namespaces:
        try:
            ...
            # If the module itself wasn't found, continue checking other
            # namespaces. If it was found but failed to import something
    ...
    if module is None:
        ...
        print(
            "Searched in: rigi_analysis.mutation, rigi_analysis.sv, "
            "rigi_analysis.workflows, rigi_analysis.utils"
        )
        sys.exit(1)
