...
**Execution Order:** You must run the mutation pipeline first to generate the required intermediate tables, followed by the SV pipeline. Each documentation file provides specific setup instructions, environmental requirements, and end-to-end examples.
...
**Required inputs (from mutation pipeline):** `mutation_merged_dir` pointing to the merged CSVs above (specifically `DBS` subdirectory for INV-DBS enrichment analyses).
...
Composes both pipelines sequentially. The mutation pipeline runs to completion first, then its `merged_analysis/` directory is automatically wired as `mutation_merged_dir` for the SV pipeline.
