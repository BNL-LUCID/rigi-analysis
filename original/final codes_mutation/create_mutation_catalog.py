...
def process_mutation_file(filepath, file_index, total_files):
    ...
    try:
        ...
        if col_map['gene'] is None:
            print(f"  ⚠ No gene column found, skipping")
            return None
...
def main():
    ...
    if catalog is not None:
        ...
        print("=" * 70)
        print(f"\nOutput files:")
        print(f"  - mutation_gene_catalog.csv (detailed by gene/dose/pattern/type)")
        print(f"  - mutation_gene_summary.csv (gene-level summary)")
